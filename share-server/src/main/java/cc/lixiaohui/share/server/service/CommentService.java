...
public class CommentService extends AbstractService {
	...
	public String getComments() throws ServiceException {
		try {
			...
			int limit = getIntParameter("limit", 20);
	...
	public String deleteComment() throws ServiceException {
		...
		try {
			...
			if (comment.getFromUser().getId() != session.getUserId()) {
	...
	public String publishComment() throws ServiceException{
		...
		try {
			...
			User fromUser = userDao.getById(session.getUserId());
			User toUser = session.getUserId() == toUserId ? fromUser : userDao.getById(toUserId);
